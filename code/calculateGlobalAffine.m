...
%     matchedPoints2 = points2(matchedFeatures(:,2),:);
    ...
    IMG1_gray = IMG1_gray(min_y - Width/2:max_y + Width/2, min_x - Width/2:max_x + Width/2);
    IMG2_gray = single(rgb2gray(IMG2));
    IMG2_gray = IMG2_gray(min_y - Width/2:max_y + Width/2, min_x - Width/2:max_x + Width/2);
    
    [f1, d1] = vl_sift(IMG1_gray);
    [f2, d2] = vl_sift(IMG2_gray);
    
    matches = vl_ubcmatch(d1, d2, 60);
...
%     showMatchedFeatures(IMG1_gray, IMG2_gray, matched_pts1, matched_pts2, 'montage');
    ...
    WarpedFrame = imwarp(IMG1, tform, 'OutputView', imref2d(size(Mask)));
